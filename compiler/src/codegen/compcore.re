  ...
  let ret =
    if (Config.no_gc^) {
      ...
      Concatlist.list_of_t(
        singleton(set_swap(wasm_mod, env, 0, arg, ~skip_incref))
...
let compile_array_op = (wasm_mod, env, arr_imm, op) => {
  ...
  | MArrayGet(idx_imm) =>
    ...
    let set_arr = () => set_swap(2, get_arr());
...
let compile_prim1 = (wasm_mod, env, p1, arg, loc): Expression.t => {
  ...
  | Throw =>
    // [TODO] When we have exception handling, revisit whether there is any GC required here
    Expression.Block.make(
      wasm_mod,
      gensym_label("throw"),
      [
